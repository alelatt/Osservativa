...
def Visualise_Multipl(board_list, lumin_treshold, board_type = '', rec_type = ''):
	...
	for ax in axes.flat:
		if i < len(board_list):
		    ...
		    ax.set_title("Luminosity Treshold " + str(int(lumin_treshold[i]*100)) + "%", fontsize = 20)
	...
	fig.savefig(fname = board_type + '_boards_' + rec_type + '.pdf', format = 'pdf')
	plt.show()
	...
	fig.savefig(fname = board_type + '_hists_' + rec_type + '.pdf', format = 'pdf')
	plt.show()

	return


def LuminDistr(list_in, list_gauss, list_lucy, board_type = ''):
	plt.figure(figsize = [10,10], dpi = 100, layout = 'tight')
	...
	plt.savefig(fname = board_type + '_lumin.pdf', format = 'pdf')
...
Visualise_Multipl(out_g, lumin_treshold = lum_tresh, board_type = board_t, rec_type = 'gauss')
Visualise_Multipl(out_l, lumin_treshold = lum_tresh, board_type = board_t, rec_type = 'lucy')
LuminDistr(lumin_in, lumin_g, lumin_l, board_type = board_t)