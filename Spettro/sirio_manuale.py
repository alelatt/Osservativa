...
def MakeMasterFlat(file_names, master_dark):
	...
	Inputs:
		file_names : list
			File names of the images to average
...
def Interactivity(image_names, lamp, flat, dark, FitFn, debug = False):
	...
	if choice == "y":
		...
		img1_raw = ndimage.rotate(fits.getdata(image_names[0], ext=0)/float(hdul1[0].header['EXPOSURE']), -rotated_ang, reshape = False) - dark

		hdul2 = fits.open(image_names[1])
		img2_raw = (fits.getdata(image_names[1], ext=0)/float(hdul2[0].header['EXPOSURE'])) - dark
...
def CheckBack(wavelength, spectrum, vega, airmass):
	"""
	...
	spectrum_corr = spectrum[mask_img]/(instr_0[mask_resp]*(trans_0[mask_resp]**airmass))
...
if __name__ == '__main__':
	...
	lamp = ((hdulamp[0].data - master_dark)*np.mean(master_flat))/float(hdulamp[0].header['EXPOSURE'])
	...
	transmittance_0, instrumental_0 = Response(wav[mask], spav1[mask], spav2[mask], spav3[mask], air1, air2, 1., 1., debug = False)

	CheckBack(wav, spav1, spav3, air1)