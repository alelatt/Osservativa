...
def Interactivity(instr = 'n'):
	...
	Outputs:
		...
		height		Height above MSL  - Value in m
...
def FindH0(h0, phi, dec):
	...
	Inputs:
		h0	Computed value of h0 (see Seth0) - Value in deg
		phi	Observer latitude - Value in deg
		dec	dec of the object at 0h on observation day - Value in deg

	Outputs:
		H0	First estimate of H0 - Value in deg
...
def InitDayFrac(ra, L, T0, H0, debug = 0):
	'''
	Sets up an array with a first estimate of the three day fractions for rising, culmination and setting (in this order)

	Inputs:
		ra		ra of the object at 0h on observation day - Value in deg
		L		Observer longitude - Value in deg
		T0		GAST (see GAST_deg) - Value in deg
		H0		First estimate of hour angle (see FindH0) - Value in deg
		debug	Debug option: debug = 1 shows these first estimates

	Outputs:
		m		Array of day fractions
...
def CorrectDayFrac(dfrac, ra_set, dec_set, phi, L, T0, DT, h0, debug = 0, RiseOrSet = True):
	'''
	Corrects first estimate for day fraction estimate. Different correction between rise/set and culmination

	Inpust:
		dfrac		Day fraction for which the correction is needed
		ra_set		Set of ra (see CoordArraySetup) - Values in deg
		dec_set		Set of dec (see CoordArraySetup) - Values in deg
		phi			Observer latitude - Value in deg
		L			Observer longitude - Value in deg
		...
		DT			TT - UT in seconds
		h0			Computed value of h0 (see Seth0) - Value in deg
		debug		Debug option: debug = 1 shows corrections at each step
		RiseOrSet	Sets correction procedure

	Outputs:
		m	Corrected day fraction
		h	Alt. - Value in deg
		A	Az. - Value in deg
...
def CorrectFracSet(frac_set, ra_set, dec_set, phi, L, T0, DT, h0, debug = 0):
	'''
	...
	Inputs:
		frac_set	Set of day fractions for which the correction is needed
		ra_set		Set of ra (see CoordArraySetup) - Values in deg
		dec_set		Set of dec (see CoordArraySetup) - Values in deg
		phi			Observer latitude - Value in deg
		L			Observer longitude - Value in deg
	...
	Outputs:
		m	Corrected day fraction set
		h	Alt. set - Values in deg
		A	Az. set - Values in deg		
	'''
...
def PlotIt(dfrac_set, ra_set, dec_set, phi, L, T0, DT, h0):
	...
	Inputs:
		dfrac_set	Set of corrected day fractions
		ra_set		Set of ra (see CoordArraySetup) - Values in deg
		dec_set		Set of dec (see CoordArraySetup) - Values in deg
		phi			Observer latitude - Value in deg
		L			Observer longitude - Value in deg
...
def TraceIt(dd, mm, yy, ra_ICRS, dec_ICRS, phi, L, height, debug = 0, obj = 'star'):
	'''
	...
	Inputs:
		...
		L			Observer Longitude  - Value in deg
		height		Height above MSL  - Value in m
		debug		Debug options: 0 for no debug info, 1 for debug info

	Outputs:
		times	Set of datetime objects containing events dates and times
		h		Set of alt. - Values in deg 
		A		Set of az. - Values in deg
	'''
...
def SunInfo(dd, mm, yy, ra_ICRS, dec_ICRS, phi, L, height, debug = 0):
	'''
	Compiles all necessary info (rise, culmination, set, start and end of twilights) for the Sun
	...
	Outputs:
		times_S_tot	Set of datetime objects containing events dates and times
		A_S			Set of az. for sunrise, culmination and sunset - Values in deg
	'''	

	times_S, h_S, A_S = TraceIt(dd, mm, yy, ra_ICRS, dec_ICRS, phi, L, height, debug, obj = 'sun')
	times_S_civ, h_S_civ, A_S_civ = TraceIt(dd, mm, yy, ra_ICRS, dec_ICRS, phi, L, height, debug, obj = 'civil')
	times_S_astro, h_S_astro, A_S_astro = TraceIt(dd, mm, yy, ra_ICRS, dec_ICRS, phi, L, height, debug, obj = 'astro')

	times_S_tot = np.array([times_S_astro[0], times_S_civ[0], times_S[0], times_S[1], times_S[2], times_S_civ[2], times_S_astro[2]])
	return (times_S_tot, A_S)
...
##################	RISULTATI	##################
times_S, A_S = SunInfo(dd, mm, yy, ra_ICRS, dec_ICRS, phi, L, height, debug = 0)

print("Sun:")
print("\tAstro Twilight End:\t %s [UTC]" %(times_S[0].strftime("%Y-%m-%d %H:%M:%S")))
print("\tCivil Twilight End:\t %s [UTC]" %(times_S[1].strftime("%Y-%m-%d %H:%M:%S")))
print("\tRising:\t\t\t %s [UTC] \t Az. from North %.2f [deg]" %(times_S[2].strftime("%Y-%m-%d %H:%M:%S"), A_S[0]))
print("\tTransit:\t\t %s [UTC] \t Az. from North %.2f [deg]" %(times_S[3].strftime("%Y-%m-%d %H:%M:%S"), A_S[1]))
print("\tSetting:\t\t %s [UTC] \t Az. from North %.2f [deg]" %(times_S[4].strftime("%Y-%m-%d %H:%M:%S"), A_S[2]))
print("\tCivil Twilight Start:\t %s [UTC]" %(times_S[5].strftime("%Y-%m-%d %H:%M:%S")))
print("\tAstro Twilight Start:\t %s [UTC]" %(times_S[6].strftime("%Y-%m-%d %H:%M:%S")))


times, h, A = TraceIt(dd, mm, yy, ra_ICRS, dec_ICRS, phi, L, height, obj = 'star', debug = 0)

print("Object:")
print("\tRising:\t\t %s [UTC] \t Alt. %.2f [deg] \t Az. from North %.2f [deg]" %(times[0].strftime("%Y-%m-%d %H:%M:%S"), h[0], A[0]))
print("\tTransit:\t %s [UTC] \t Alt. %.2f [deg] \t Az. from North %.2f [deg] \t Airmass %.2f" %(times[1].strftime("%Y-%m-%d %H:%M:%S"), h[1], A[1], 1./np.cos((90 - h[1])*np.pi/180)))
